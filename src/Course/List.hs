...
filter ::
  ...
  -> List a
  -> List a
filter f l = foldRight (\h b -> if (f h) then (h:.b) else b) Nil l  
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight (\h b -> applyOptional (mapOptional (:.) h) b) (Full Nil)
